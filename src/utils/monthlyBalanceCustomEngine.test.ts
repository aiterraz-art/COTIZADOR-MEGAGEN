...
describe('monthlyBalanceCustomEngine', () => {
  it('mapea las cuentas reales del balance, conserva signos definidos y calcula totales', () => {
    ...
    expect(result.warnings).toEqual([
      'El balance no cuadra: TOTAL ASSETS difiere de TOTAL LIABILITIES AND EQUITY por -285.',
    ]);
  ...
  it('usa la linea Resultado del balance cuando no existe ER cargado', () => {
    ...
    expect(result.netIncomeDifferenceCLP).toBe(0);
    expect(result.balanceDifferenceCLP).toBe(-400);
    expect(result.warnings).toEqual([
      'El balance no cuadra: TOTAL ASSETS difiere de TOTAL LIABILITIES AND EQUITY por -400.',
    ]);
  });
